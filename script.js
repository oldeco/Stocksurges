...
const API_KEY = '0fe49cfed728cedc398677d2f019d072'; // your REST key
...
async function fetchStock(sym) {
  ...
  // 1. Snapshot for today’s close, prev‑day, % change, volume
  let snap;
  try {
    snap = await fetch(`https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers/${sym}?apiKey=${API_KEY}`)
      .then(r=>r.json());
  ...
  if (!snap.ticker) {
    console.warn('no snapshot data', snap);
  ...
  // 2. Aggregates for last 30 days
  const end   = new Date().toISOString().slice(0,10);
  const start = new Date(Date.now() - 30*24*60*60*1000).toISOString().slice(0,10);
  let aggs;
  try {
    aggs = await fetch(`https://api.polygon.io/v2/aggs/ticker/${sym}/range/1/day/${start}/${end}?apiKey=${API_KEY}`)
      .then(r=>r.json());
    console.log('Aggregates:', aggs);
  } catch(err) {
    console.error('aggs error', err);
    return;
  }
